...
class Example(wx.Frame):
    ...
    def InitUI(self):       
        ...
        buttonGuest = wx.Button(panel, label="Гость",pos=(40, 120), size=(90, 28))
        buttonGuestGS = wx.Button(panel, label="GS Гость",pos=(183, 120), size=(90, 28))
    ...
    def guestbutton(self, event):
        ...
        """ Редактирование *.bat файла для группы radiusGS """
        
        file.write('''chcp 1251 >nul
net user %s %s /add /fullname:"%s" /comment:"%s %s" /expires:"%s"
net localgroup radiusGS %s /add
    ''' % (login, pw, logvar, limiteddate, organization, limiteddate, login))
        file.close()
        os.rename('testfilemeow.txt', '%swf.bat' %login)
#LOGS
        runas = os.environ["USERNAME"]
        curdate = datetime.datetime.now()
        currentdate = curdate.strftime('%d.%m.%Y')
...
---------------------------------------------------------------------------------------------------
''' % (
            runas, currentdate, logvar, organization, limiteddate, login,))
        file.close()
#Редактирование Microsoft Word и вывод на печать
        opendoc = open('manual.docx', 'rb')
        document = Document(opendoc)
        document.paragraphs[0].insert_paragraph_before('Имя пользователя: «s-sd\%s»            Пароль: «%s»' %(login, pw))
    ...
    def guestbuttongs(self, event):
        ...
        """ Редактирование *.bat файла для группы GuestWF """
        file.write('''chcp 1251 >nul
net user %s %s /add /fullname:"%s" /comment:"%s %s" /expires:"%s"
net localgroup GuestWF %s /add
    ''' % (login, pw, logvar, limiteddate, organization, limiteddate, login))
        file.close()
        os.rename('testfilemeow.txt', '%swf.bat' %login)
